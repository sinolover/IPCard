...
Begin VB.Form frmAlertDate 
   Caption         =   "日期警戒 近期没有使用记录的账号信息"
   ClientHeight    =   6795
   ClientLeft      =   960
   ClientTop       =   1020
   ClientWidth     =   9480
   ...
   Begin MSDataGridLib.DataGrid dgCount 
      Bindings        =   "frmDateAlert.frx":0000
      Height          =   4335
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmAlertDate"
...
'refreshAlertDate Me.dgCount.Columns
strAlert = GetReg("AlertDate", "2")
If Not IsNumeric(strAlert) Then strAlert = "2"
strSql = "Select * from cxAlertMoney where AlertMoney > 0 and  (" _
         & " LastDate <=  " _
         & Format(DateAdd("d", Val(strAlert) * (-1), Date), "YYYY-MM-DD") _
         & "  or LastDate = null ) order by LastDate DESC, WkrName"
...
End Sub
Private Sub refreshAlertDate(col As Columns)
With col
 .Item("费用总额").Width = 0
 .Item("充值总额").Width = 0
End With
